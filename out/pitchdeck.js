...
if(typeof novel=='undefined') novel = {}
novel.Scene = function(p) {
	if( p === $_ ) return;
	game2.Playground.call(this);
	this.animations = new Hash();
	this.elapsed_frames = 0;
}
novel.Scene.__name__ = ["novel","Scene"];
novel.Scene.__super__ = game2.Playground;
for(var k in game2.Playground.prototype ) novel.Scene.prototype[k] = game2.Playground.prototype[k];
novel.Scene.prototype.animations = null;
novel.Scene.prototype.interactions = null;
novel.Scene.prototype.elapsed_frames = null;
novel.Scene.prototype.Step = function() {
	game2.Playground.prototype.Step.call(this);
	var $it0 = this.animations.iterator();
	while( $it0.hasNext() ) {
...
novel.Scene.prototype.Load = function(s_data) {
	var me = this;
	this.animations = new Hash();
	var _g = 0, _g1 = s_data.animations;
	while(_g < _g1.length) {
		var data = _g1[_g];
		++_g;
		var anime = new novel.Animation(this,data);
		...
		if(s_data.interactions.exists(data.animation_id)) {
			...
			anime.Mouseleave(lambda_callback_generator(anime,interaction[0].mouseleave));
			anime.Mouseover(lambda_callback_generator(anime,interaction[0].mouseover));
			anime.Mousemove(lambda_callback_generator(anime,interaction[0].mousemove));
		}
	}
}
novel.Scene.prototype.AddAnimation = function(animation) {
	this.animations.set(animation.Id(),animation);
}
novel.Scene.prototype.Serialize = function() {
	throw "Not Implemented Exception";
}
novel.Scene.prototype.Show = function() {
	var _g = 0, _g1 = this.toys;
	while(_g < _g1.length) {
		var toy = _g1[_g];
		++_g;
		toy.Show();
...
novel.Scene.prototype.Hide = function() {
	var _g = 0, _g1 = this.toys;
	while(_g < _g1.length) {
		var toy = _g1[_g];
		++_g;
...
novel.Scene.prototype.__class__ = novel.Scene;
novel.Novel = function(p) {
	if( p === $_ ) return;
	novel.Scene.call(this);
	this.scenes = [];
	this.buttons = new Hash();
}
novel.Novel.__name__ = ["novel","Novel"];
novel.Novel.__super__ = novel.Scene;
...
novel.Novel.prototype.LoadNovel = function(s_data,ui_data) {
...
buildingblocks.Canvas.ConvertPosition = function(p) {
	return { x : p.x / 100 * buildingblocks.Canvas.Configuration.effective_width + buildingblocks.Canvas.Configuration.band_width, y : p.y / 100 * buildingblocks.Canvas.Configuration.effective_height + buildingblocks.Canvas.Configuration.band_height};
}
buildingblocks.Canvas.ConvertSize = function(s) {
	return { width : s.width / 100 * buildingblocks.Canvas.Configuration.effective_width, height : s.height / 100 * buildingblocks.Canvas.Configuration.effective_height};
}
...
buildingblocks.CoreObject = function(p) {
	...
	dispatch.EventLaser.Register(this);
}
...
buildingblocks.CoreObject.prototype.Listen4 = function(eventname,callb) {
	if(!this.delegate_hash.exists(eventname)) this.delegate_hash.set(eventname,[function(e) {
		haxe.Log.trace(e,{ fileName : "CoreObject.hx", lineNumber : 20, className : "buildingblocks.CoreObject", methodName : "Listen4"});
	}]);
...
buildingblocks.Div.prototype.Hide = function() {
	this.self.hide();
}
buildingblocks.Div.prototype.Show = function() {
	this.self.show();
}
...
game2.Toy.prototype.Hide = function() {
	if(this.image != null) this.image.Hide();
	if(this.text != null) this.text.Hide();
	if(this.div != null) this.div.Hide();
}
game2.Toy.prototype.Show = function() {
	if(this.image != null) this.image.Show();
	if(this.text != null) this.text.Show();
	if(this.div != null) this.div.Show();
}
...
haxespec.FuryTestCase.prototype.assertMathEqual = function(a,b,pos) {
...
pitchdeckspec.PitchDeckSpec.prototype.fasdftestConverterLinear = function() {
	var a = tools.Random.Vector(7);
	var b = tools.Random.Vector(7);
	var linear = tools.Converter.GetLine(a,b);
	this.assertMathEqual(linear(0.0),a,{ fileName : "PitchDeckSpec.hx", lineNumber : 33, className : "pitchdeckspec.PitchDeckSpec", methodName : "fasdftestConverterLinear"});
	this.assertMathEqual(linear(100.0),b,{ fileName : "PitchDeckSpec.hx", lineNumber : 34, className : "pitchdeckspec.PitchDeckSpec", methodName : "fasdftestConverterLinear"});
}
pitchdeckspec.PitchDeckSpec.prototype.asdftestConverterQuad = function() {
	var a = tools.Random.Vector(8);
	var b = tools.Random.Vector(8);
	var c = tools.Random.Vector(8);
	var quad = tools.Converter.GetQuadratic(a,b,c);
	this.assertMathEqual(quad(-1.0),a,{ fileName : "PitchDeckSpec.hx", lineNumber : 42, className : "pitchdeckspec.PitchDeckSpec", methodName : "asdftestConverterQuad"});
	this.assertMathEqual(quad(0.0),b,{ fileName : "PitchDeckSpec.hx", lineNumber : 43, className : "pitchdeckspec.PitchDeckSpec", methodName : "asdftestConverterQuad"});
	this.assertMathEqual(quad(1.0),c,{ fileName : "PitchDeckSpec.hx", lineNumber : 44, className : "pitchdeckspec.PitchDeckSpec", methodName : "asdftestConverterQuad"});
}
...
tools.Random.Vector = function(len) {
...
novel.Animation = function(scene,data) {
	if( scene === $_ ) return;
	game2.Toy.call(this,scene,noveldata.AnimationHelper.GetToy(data,0));
	this.animation_data = data;
	scene.AddAnimation(this);
}
novel.Animation.__name__ = ["novel","Animation"];
novel.Animation.__super__ = game2.Toy;
for(var k in game2.Toy.prototype ) novel.Animation.prototype[k] = game2.Toy.prototype[k];
novel.Animation.prototype.animation_data = null;
novel.Animation.prototype.Id = function() {
	return this.animation_data.animation_id;
}
novel.Animation.prototype.Step = function(time) {
	var ad = noveldata.AnimationHelper.GetToy(this.animation_data,time);
	if(ad.image_id != null) {
...
novel.Animation.prototype.Position = function(p) {
	if(this.animation_data.image_id != null) return this.image.Position(p);
	if(this.animation_data.text_id != null) return this.text.Position(p);
	if(this.animation_data.div_id != null) throw "Not implemented exception : Positioning a div";
	return p;
}
novel.Animation.prototype.Size = function(s) {
	if(this.animation_data.image_id != null) return this.image.Size(s);
	if(this.animation_data.text_id != null) return this.text.Size(s);
	if(this.animation_data.div_id != null) throw "Not implemented exception : Positioning a div";
	return s;
}
novel.Animation.prototype.Angle = function(a) {
	if(this.animation_data.image_id != null) return this.image.Angle(a);
	if(this.animation_data.text_id != null) return this.text.Angle(a);
	if(this.animation_data.div_id != null) throw "Not implemented exception : Positioning a div";
	return a;
}
novel.Animation.prototype.__class__ = novel.Animation;
...
buildingblocks.Element.__super__ = buildingblocks.CoreObject;
for(var k in buildingblocks.CoreObject.prototype ) buildingblocks.Element.prototype[k] = buildingblocks.CoreObject.prototype[k];
buildingblocks.Element.prototype.id = null;
buildingblocks.Element.prototype.index = null;
buildingblocks.Element.prototype.has_mouse_flag = null;
buildingblocks.Element.prototype.visible_flag = null;
...
buildingblocks.Element.prototype.Size = function(s) {
	return s;
}
buildingblocks.Element.prototype.Position = function(p) {
	return p;
}
buildingblocks.Element.prototype.Angle = function(a) {
	return a;
}
buildingblocks.Element.prototype.Click = function(cb) {
	var me = this;
	buildingblocks.Interaction.Register("mouseclick",function(e) {
		if(!me.visible_flag) return;
		var point = { x : e.pageX + 0.0, y : e.pageY + 0.0};
		var box_position = buildingblocks.Canvas.ConvertPosition(me.Position());
		var box_size = buildingblocks.Canvas.ConvertSize(me.Size());
		if(tools.Measure.PointInBox(point,box_position,box_size)) cb(e);
	});
}
buildingblocks.Element.prototype.Mouseover = function(cb) {
	var me = this;
	buildingblocks.Interaction.Register("mousemove",function(e) {
		if(!me.visible_flag) return;
		var point = { x : e.pageX + 0.0, y : e.pageY + 0.0};
		var box_position = buildingblocks.Canvas.ConvertPosition(me.Position());
		var box_size = buildingblocks.Canvas.ConvertSize(me.Size());
		if(tools.Measure.PointInBox(point,box_position,box_size)) {
			me.has_mouse_flag = true;
			cb(e);
		}
	});
}
buildingblocks.Element.prototype.Mouseleave = function(cb) {
	var me = this;
	buildingblocks.Interaction.Register("mousemove",function(e) {
		if(!me.visible_flag) return;
		var point = { x : e.pageX + 0.0, y : e.pageY + 0.0};
		...
		var box_size = buildingblocks.Canvas.ConvertSize(me.Size());
		if(!tools.Measure.PointInBox(point,box_position,box_size)) {
			if(me.has_mouse_flag == true) {
				me.has_mouse_flag = false;
				cb(e);
			}
		}
	});
}
buildingblocks.Element.prototype.Mousemove = function(cb) {
	var me = this;
	buildingblocks.Interaction.Register("mousemove",function(e) {
		if(!me.visible_flag) return;
		cb(e);
...
buildingblocks.Text.prototype.Text = function(raw_txt) {
	if(raw_txt != null) this.text_data.raw_text = raw_txt;
	return this.text_data.raw_text;
}
buildingblocks.Text.prototype.Append = function(raw_txt) {
...
buildingblocks.Text.prototype.Size = function(s) {
	if(s != null) this.text_data.text_size = Math.floor(s.height);
	var w = buildingblocks.Canvas.Context.measureText(this.text_data.raw_text).width + 0.0;
	return { width : w, height : this.text_data.text_size + 0.0};
}
buildingblocks.Text.prototype.Position = function(p) {
	if(p != null) this.text_data.position = p;
	return this.text_data.position;
}
buildingblocks.Text.prototype.Angle = function(a) {
	if(a != null) this.text_data.angle = a;
...
specfactory.NovelFactory.AnimationData = function(type) {
	...
	case "image":
		...
		output.image_id = "0";
		output.image_path = function(t) {
			return { x : 25 + rand * Math.cos(t / 50), y : 25 + rand * Math.sin(t / 50)};
		};
		var angle = 0.0;
		...
		break;
	case "text":
		var text_data = specfactory.BuildingBlocksFactory.TextData();
		var rand = tools.Random.Number(30);
		output.text_id = "0";
		output.text_path = function(t) {
			return { x : 25 + rand * Math.cos(t / 50), y : 25 + rand * Math.sin(t / 50)};
		};
		var angle = 0.0;
...
pitchdeck.PitchDeck.prototype.LoadUI = function(data) {
	this.p_buttons(data);
}
...
tools.Converter = function() { }
tools.Converter.__name__ = ["tools","Converter"];
tools.Converter.GetLine = function(a,b) {
...
tools.Converter.GetQuadratic = function(a,b,c) {
	...
	var X = [a,b,c];
	var K = [tools.Converter.GetZeroes(a.length),tools.Converter.GetZeroes(a.length),tools.Converter.GetZeroes(a.length)];
	var _g = 0;
	while(_g < 3) {
		var i = _g++;
		var _g1 = 0;
	...
	return function(t) {
		var output = tools.Converter.GetZeroes(a.length);
		var _g1 = 0, _g = output.length;
		while(_g1 < _g) {
			var k = _g1++;
...
tools.Converter.GetZeroes = function(len) {
	var l = len == null?1:len;
...
tools.Converter.prototype.__class__ = tools.Converter;
...
specfactory.BuildingBlocksFactory.CanvasData = function() {
	return { ratio : tools.Random.Number(300) / 100, width : tools.Random.Number(1440) + 0.0, height : tools.Random.Number(900) + 0.0};
}
specfactory.BuildingBlocksFactory.ImageData = function() {
	return { source : "madotsuki.png", source_position : { x : 0.0, y : 0.0}, source_size : { width : 60.0, height : 60.0}, position : { x : tools.Random.Number(100) + 0.0, y : tools.Random.Number(100) + 0.0}, size : { width : tools.Random.Number(10) + 10.0, height : tools.Random.Number(10) + 10.0}, angle : tools.Random.Number(360) / 180 * Math.PI, opacity : 1.0, frame : 0.0};
}
...
specfactory.BuildingBlocksFactory.TextData = function() {
	...
	var font = ["Arial","sans-serif","Times New Roman"][tools.Random.Number(3)];
	return { raw_text : tools.Random.Text(25), text_font : font, text_size : tools.Random.Number(100), text_color : color, outline_color : outline, align : ["start","end","left","right","center"][tools.Random.Number(5)], baseline : ["alphabetic","bottom","hanging","ideographic","middle","top"][tools.Random.Number(6)], position : { x : tools.Random.Number(100) + 0.0, y : tools.Random.Number(100) + 0.0}, angle : tools.Random.Number(360) / 180 * Math.PI, opacity : 1.0};
}
...
noveldata.Interaction = { __ename__ : ["noveldata","Interaction"], __constructs__ : ["None","Inflate","Deflate","Hide","Show","Next","Back"] }
noveldata.Interaction.None = ["None",0];
noveldata.Interaction.None.toString = $estr;
noveldata.Interaction.None.__enum__ = noveldata.Interaction;
noveldata.Interaction.Inflate = ["Inflate",1];
noveldata.Interaction.Inflate.toString = $estr;
noveldata.Interaction.Inflate.__enum__ = noveldata.Interaction;
noveldata.Interaction.Deflate = ["Deflate",2];
noveldata.Interaction.Deflate.toString = $estr;
noveldata.Interaction.Deflate.__enum__ = noveldata.Interaction;
noveldata.Interaction.Hide = ["Hide",3];
noveldata.Interaction.Hide.toString = $estr;
noveldata.Interaction.Hide.__enum__ = noveldata.Interaction;
noveldata.Interaction.Show = ["Show",4];
noveldata.Interaction.Show.toString = $estr;
noveldata.Interaction.Show.__enum__ = noveldata.Interaction;
noveldata.Interaction.Next = ["Next",5];
noveldata.Interaction.Next.toString = $estr;
noveldata.Interaction.Next.__enum__ = noveldata.Interaction;
noveldata.Interaction.Back = ["Back",6];
noveldata.Interaction.Back.toString = $estr;
...
specfactory.PitchDeckFactory.UIData = function() {
...
specfactory.PitchDeckFactory.SlideData = function(type) {
	...
	case "problem":
		...
		animation_data.image_data = function(f) {
...
dispatch.EventCannon.Listen = function(name,cb,pos) {
	if(!dispatch.EventCannon.event_storage.exists(name)) dispatch.EventCannon.event_storage.set(name,[function(e) {
		haxe.Log.trace(e,pos);
	}]);
	dispatch.EventCannon.event_storage.get(name).push(cb);
}
dispatch.EventCannon.Fire = function(e) {
	if(!dispatch.EventCannon.event_storage.exists(e.name)) return;
	var _g = 0, _g1 = dispatch.EventCannon.event_storage.get(e.name);
	while(_g < _g1.length) {
		var handler = _g1[_g];
		++_g;
		handler(e);
...
dispatch.EventCannon.prototype.__class__ = dispatch.EventCannon;
...
buildingblocks.Image.prototype.Position = function(p) {
	if(p != null) this.image_data.position = p;
	return this.image_data.position;
}
buildingblocks.Image.prototype.Size = function(s) {
	if(s != null) this.image_data.size = s;
...
buildingblocks.Canvas.Self = (function() {
	...
	if(body.length <= 0) body.append("<canvas id='canvas' width='800' height='600'>Something went terribly wrong</canvas>");
	var c = js.Lib.document.getElementById("canvas");
	if(c.getAttribute("width") == null) c.setAttribute("width","800");
	if(c.getAttribute("height") == null) c.setAttribute("height","600");
	return c;
})();
buildingblocks.Canvas.Context = (function() {
	if(buildingblocks.Canvas.Self == null) {
		haxe.Log.trace("You need to place this code AFTER the <canvas> tag in body in order for this to work!",{ fileName : "Canvas.hx", lineNumber : 28, className : "buildingblocks.Canvas"});
...
buildingblocks.Canvas.Configuration = (function() {
	var c = new js.JQuery("canvas#canvas");
	var config = { ratio : 1.6, width : Std.parseFloat(c.attr("width")), height : Std.parseFloat(c.attr("height")), effective_width : 0.0, effective_height : 0.0, band_height : 0.0, band_width : 0.0};
...
buildingblocks.Interaction.Interactions = (function() {
	var cb_hash = new Hash();
	var canvas = new js.JQuery("canvas#canvas");
	var body = new js.JQuery("body");
	if(canvas == null || canvas.length == 0) throw "Canvas not found and I don't get paid enough to make one error.";
	if(body == null || body.length == 0) throw "Body not found and I am not even going to bother with trying to create one.";
	var _g = 0, _g1 = buildingblocks.Interaction.Actions;
	try {
		while(_g < _g1.length) {
			var action = _g1[_g];
			++_g;
			cb_hash.set(action,new Hash());
			var lambda_callback = (function(a) {
				return function(e) {
					var $it0 = cb_hash.get(a).iterator();
					while( $it0.hasNext() ) {
						var cb = $it0.next();
						cb(e);
					}
